...
export async function GET(request: Request) {
  ...
  try {const result=await importLatestStories();await recordHealth("daily news import",true,JSON.stringify(result)).catch(error=>console.error("Health logging failed",error));return NextResponse.json(result);}
