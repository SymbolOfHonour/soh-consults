...
export default async function AdminPage() {
  ...
  const thisWeek = stories.filter(s => s.status === "published" && +new Date(s.updated_at) >= now - 7*86400000).length;
  const thisMonth = stories.filter(s => s.status === "published" && +new Date(s.updated_at) >= now - 30*86400000).length;
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-950">
    ...
    <section className="mx-auto max-w-[1450px] px-5 py-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat label="Published" value={counts.published} note={`${thisWeek} updated in last 7 days`} /><Stat label="Drafts" value={counts.draft} note="Waiting for review" tone="amber" /><Stat label="Approved" value={counts.approved} note="Ready to publish" tone="blue" /><Stat label="Total Updates" value={counts.total} note={`${counts.archived} archived · ${counts.rejected} rejected`} tone="gray" /></div>
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[.16em] text-green-700">Publishing snapshot</p><div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-xl bg-gray-50 p-3"><p className="text-2xl font-black">{thisWeek}</p><p className="text-xs font-bold text-gray-500">Updated 7 days</p></div><div className="rounded-xl bg-gray-50 p-3"><p className="text-2xl font-black">{thisMonth}</p><p className="text-xs font-bold text-gray-500">Updated 30 days</p></div></div><h4 className="mt-5 font-black">Content breakdown</h4><div className="mt-2 space-y-2">{categories.map(([name,count]) => <div key={name} className="flex justify-between text-sm"><span className="font-bold text-gray-600">{name}</span><span className="font-black">{count}</span></div>)}</div></section>
