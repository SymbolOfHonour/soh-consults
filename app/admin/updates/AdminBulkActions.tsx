...
export default function AdminBulkActions() {
  ...
  async function changeStatus(status: "published" | "archived" | "rejected") {
    const chosen = stories.filter((story) => selected.has(story.id));
    const eligible = status === "rejected"
      ? chosen.filter((story) => story.status !== "published" && story.status !== "archived" && story.status !== "rejected")
      : status === "archived"
        ? chosen.filter((story) => story.status !== "archived")
        : chosen.filter((story) => story.status !== "published");
    if (!eligible.length) { setMessage("No selected updates are eligible for this action."); return; }
    const verb = status === "published" ? "Publish" : status === "archived" ? "Archive" : "Reject";
    if (!confirm(`${verb} ${eligible.length} selected update${eligible.length === 1 ? "" : "s"}?`)) return;
  ...
  const toolbar = mounted && selected.size > 0 ? createPortal(
    <div className="fixed left-1/2 top-24 z-[100] w-[calc(100%-2rem)] max-w-6xl -translate-x-1/2 rounded-2xl border-2 border-green-400 bg-white p-4 shadow-2xl">
      <div className="flex flex-wrap items-center justify-center gap-2">
        <span className="mr-2 rounded-lg bg-green-700 px-3 py-2 text-sm font-black text-white">{selected.size} selected</span>
        ...
        <button type="button" disabled={busy} onClick={() => void changeStatus("published")} className="rounded-xl bg-green-700 px-4 py-2 text-sm font-black text-white disabled:opacity-50">Publish Selected</button>
        ...
        <button type="button" disabled={busy} onClick={() => void permanentDelete()} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-black text-white disabled:opacity-50">Delete Selected</button>
      </div>
      <p className="mt-2 text-center text-xs font-bold text-gray-600">Select All selects every update currently visible in this tab/filter. Delete Selected permanently deletes all selected updates after confirmation.</p>
