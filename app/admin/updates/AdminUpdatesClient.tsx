...
export default function AdminUpdatesClient() {
  ...
  async function upload(storyId: string, file: File, kind: "image" | "document") {
    ...
    if (!response.ok) setMessage(data.error || "Upload failed.");
      ...
      setStories(current => current.map(story => story.id === storyId ? { ...story, ...(kind === "image" ? { image_url: data.url } : { document_url: data.url, document_name: data.name }) } : story));
      setMessage(`${file.name} uploaded. Click Save Changes to attach it to this story.`);
    }
    setBusy(false);
  }
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-900"><header className="border-b bg-white"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-5 py-5"><div className="flex items-center gap-3"><img src="/soh-logo.jpg" alt="" className="h-14 w-auto" /><div><p className="text-xs font-black uppercase tracking-widest text-green-700">Private dashboard</p><h1 className="text-xl font-black">Update Approval Queue</h1></div></div><button onClick={importNow} disabled={busy} className="rounded-xl bg-green-700 px-5 py-3 text-sm font-black text-white disabled:opacity-60">{busy ? "Please wait..." : "Check Sources Now"}</button></div></header><section className="mx-auto max-w-7xl px-5 py-8"><div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-950"><b>Approval safety:</b> Drafts and approved stories remain hidden from visitors. A story only becomes public after you deliberately select Published.</div>{message && <p className="mt-4 rounded-xl bg-white p-4 text-sm font-bold shadow-sm">{message}</p>}<div className="my-6 flex flex-wrap gap-2">{(["all", ...statuses] as const).map(status => <button key={status} onClick={() => setFilter(status)} className={`rounded-full px-4 py-2 text-sm font-black capitalize ${filter === status ? "bg-green-700 text-white" : "border bg-white"}`}>{status} ({status === "all" ? stories.length : stories.filter(item => item.status === status).length})</button>)}</div><div className="space-y-6">{shown.map(story => <article key={story.id} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-gray-500">Private import reference</p><p className="text-sm font-black text-green-700">{story.source_name}</p></div><span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-black uppercase">{story.status}</span></div><a href={story.source_url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex break-all text-sm font-bold text-blue-700 underline">Open private import reference</a><p className="mt-2 text-xs font-bold text-gray-500">This imported source is visible only inside the private dashboard.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold md:col-span-2">Headline<input value={story.title} onChange={event => change(story.id, "title", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><label className="text-sm font-bold">Institution<input value={story.institution} onChange={event => change(story.id, "institution", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><label className="text-sm font-bold">Category<select value={story.category} onChange={event => change(story.id, "category", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal"><option>Admission</option><option>Admission List</option><option>JAMB</option><option>O&apos;Level</option><option>Scholarship</option><option>Opportunity</option><option>NYSC</option><option>LASU Updates</option></select></label><label className="text-sm font-bold md:col-span-2">S.O.H summary<textarea rows={3} value={story.summary} onChange={event => change(story.id, "summary", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><label className="text-sm font-bold md:col-span-2">Full update<textarea rows={8} value={story.details} onChange={event => change(story.id, "details", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><label className="text-sm font-bold">Deadline text<input value={story.deadline || ""} onChange={event => change(story.id, "deadline", event.target.value)} placeholder="e.g. 18 September 2026 at 11:59 PM" className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><label className="text-sm font-bold">Deadline date<input type="date" value={story.deadline_iso || ""} onChange={event => change(story.id, "deadline_iso", event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3 font-normal" /></label><div className="rounded-2xl border border-green-200 bg-green-50 p-4"><p className="text-sm font-black">Optional official public source</p><label className="mt-3 block text-xs font-bold">Official source name<input value={story.official_source_name || ""} onChange={event => change(story.id, "official_source_name", event.target.value)} placeholder="e.g. WAEC" className="mt-2 w-full rounded-xl border bg-white px-4 py-3 text-sm font-normal" /></label><label className="mt-3 block text-xs font-bold">Official source link<input type="url" value={story.official_source_url || ""} onChange={event => change(story.id, "official_source_url", event.target.value)} placeholder="https://..." className="mt-2 w-full rounded-xl border bg-white px-4 py-3 text-sm font-normal" /></label><p className="mt-2 text-xs text-gray-600">Leave both blank to show no source publicly.</p></div><div className="rounded-2xl border bg-gray-50 p-4"><p className="text-sm font-black">Featured image</p>{story.image_url && <img src={story.image_url} alt="Preview" className="mt-3 h-40 w-full rounded-xl object-cover" />}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={event => { const file = event.target.files?.[0]; if (file) void upload(story.id, file, "image"); }} className="mt-3 block w-full text-sm" />{story.image_url && <button type="button" onClick={() => change(story.id, "image_url", "")} className="mt-3 text-sm font-black text-red-700">Remove image</button>}<p className="mt-2 text-xs text-gray-500">JPG, PNG or WebP. Maximum 5 MB.</p></div><div className="rounded-2xl border bg-gray-50 p-4 md:col-start-2"><p className="text-sm font-black">Supporting PDF</p>{story.document_url && <a href={story.document_url} target="_blank" rel="noopener noreferrer" className="mt-3 block break-all text-sm font-bold text-blue-700 underline">{story.document_name || "View attached PDF"}</a>}<input type="file" accept="application/pdf" disabled={busy} onChange={event => { const file = event.target.files?.[0]; if (file) void upload(story.id, file, "document"); }} className="mt-3 block w-full text-sm" />{story.document_url && <button type="button" onClick={() => { change(story.id, "document_url", ""); change(story.id, "document_name", ""); }} className="mt-3 text-sm font-black text-red-700">Remove PDF</button>}<p className="mt-2 text-xs text-gray-500">PDF only. Maximum 10 MB.</p></div></div><div className="mt-5 flex flex-wrap gap-3"><button disabled={busy} onClick={() => save(story)} className="rounded-xl border border-green-700 px-4 py-2 text-sm font-black text-green-700">Save Changes</button><button disabled={busy} onClick={() => save(story, "approved")} className="rounded-xl bg-green-700 px-4 py-2 text-sm font-black text-white">Approve</button><button disabled={busy} onClick={() => save(story, "published")} className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-black text-white">Publish</button><button disabled={busy} onClick={() => save(story, "rejected")} className="rounded-xl bg-red-100 px-4 py-2 text-sm font-black text-red-800">Reject</button></div></article>)}{!shown.length && <div className="rounded-3xl border border-dashed bg-white p-10 text-center font-bold text-gray-500">No stories in this section.</div>}</div></section></main>;
}
