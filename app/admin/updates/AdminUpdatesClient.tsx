...
export default function AdminUpdatesClient() {
  ...
  if (authenticated === null) return <main className="grid min-h-screen place-items-center bg-gray-50"><p className="font-bold text-green-800">Loading approval queue...</p></main>;
  if (!authenticated) return <main className="grid min-h-screen place-items-center bg-gray-50 p-5"><form onSubmit={login} className="w-full max-w-md rounded-3xl border border-gray-200 bg-white p-8 shadow-xl"><img src="/soh-logo.jpg" alt="S.O.H CONSULTS" className="mx-auto h-20 w-auto" /><h1 className="mt-6 text-center text-2xl font-black">Private Update Dashboard</h1><p className="mt-2 text-center text-sm text-gray-600">Enter the admin password to review imported stories.</p><input type="password" value={password} onChange={event => setPassword(event.target.value)} required className="mt-6 w-full rounded-xl border border-gray-300 px-4 py-3 outline-none focus:border-green-600" placeholder="Admin password" /><button disabled={busy} className="mt-4 w-full rounded-xl bg-green-700 px-5 py-3 font-black text-white disabled:opacity-60">{busy ? "Checking..." : "Open Dashboard"}</button>{message && <p className="mt-4 text-center text-sm font-bold text-red-700">{message}</p>}</form></main>;
