...
export default function RecoveryClient(){
 ...
 return <div>
  ...
  {tab==="trash"&&<div className="space-y-3">
   ...
   {data.trash.length?data.trash.map(s=>{return <article key={s.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 flex-1 items-start gap-3"><input type="checkbox" aria-label={`Select ${s.title}`} checked={selected.includes(s.id)} disabled={busy} onChange={event=>toggle(s.id,event.target.checked)} className="mt-1 h-5 w-5 shrink-0"/><div className="min-w-0"><h3 className="break-words font-black">{s.title}</h3><p className="mt-1 text-xs font-bold text-gray-500">Moved {new Date(s.updated_at).toLocaleString("en-NG")} · Kept in Trash until you permanently delete it.</p></div></div><div className="flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={()=>void act("restore",s.id)} className="rounded-xl bg-green-700 px-4 py-2 text-sm font-black text-white disabled:opacity-40">Restore</button><button type="button" disabled={busy} onClick={()=>permanent(s.id)} className="rounded-xl bg-red-600 px-4 py-2 text-sm font-black text-white disabled:opacity-40">Delete Forever</button></div></div></article>}):<p className="rounded-2xl border bg-white p-6 text-sm text-gray-500">Trash is empty.</p>}
  </div>}
