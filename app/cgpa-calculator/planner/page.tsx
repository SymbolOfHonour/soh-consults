...
export default function CgpaPlannerPage() {
  ...
  const [targetPeriod, setTargetPeriod] = useState<"semester" | "session" | "future">("semester");
  const [targetEndStage, setTargetEndStage] = useState("");
  const [targetFutureCourses, setTargetFutureCourses] = useState<Record<string, TargetCourse[]>>({});
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (data.ctnup) setCtnup(data.ctnup); setCtcp(""); if (data.targetCgpa) setTargetCgpa(data.targetCgpa);
  ...
  const targetFutureStages = stageLevels.slice(Math.max(0, stageLevels.indexOf(startStage)));
  const effectiveTargetEndStage = targetEndStage && targetFutureStages.includes(targetEndStage) ? targetEndStage : targetFutureStages[Math.min(1, targetFutureStages.length - 1)] || startStage;
  const targetFuturePeriods = useMemo(() => {
    if (targetPeriod !== "future") return [] as { key: string; title: string }[];
    const startIndex = Math.max(0, stageLevels.indexOf(startStage));
    const endIndex = Math.max(startIndex, stageLevels.indexOf(effectiveTargetEndStage));
    const periods: { key: string; title: string }[] = [];
    for (let i=startIndex;i<=endIndex;i++) {
      const level=stageLevels[i];
      if (i===startIndex && startTerm==="2nd Semester") periods.push({key:`${level}|2nd Semester`,title:`${level} 2nd Semester Courses`});
      else {
        periods.push({key:`${level}|1st Semester`,title:`${level} 1st Semester Courses`});
        periods.push({key:`${level}|2nd Semester`,title:`${level} 2nd Semester Courses`});
      }
    }
    return periods;
  }, [targetPeriod, startStage, startTerm, effectiveTargetEndStage, stageLevels.join("|")]);
  ...
  const targetSemesterPlan = useMemo(() => {
    ...
    const futureUnits = targetFuturePeriods.reduce((sum, period) => sum + (targetFutureCourses[period.key] || []).filter((course) => course.code.trim()).reduce((s, course) => s + Math.max(0, Number(course.units) || 0), 0), 0);
    const units = targetPeriod === "future" ? futureUnits : targetPeriod === "session" ? firstUnits + secondUnits : firstUnits;
    const target = Math.min(maxPoint, Math.max(0, Number(targetCgpa) || 0));
    const exact = false;
  ...
  function savePlan() {
    window.localStorage.setItem("soh-cgpa-planner-v3", JSON.stringify({ system, entryMode, programmeYears, studentName, programme, currentCgpa, ctnup, targetCgpa, semesters }));
  ...
  const targetStatus = !targetSemesterPlan.oldUnits || !currentCgpa.trim() ? "Enter your current CGPA and completed course units first."
    ...
    : targetSemesterPlan.requiredGpa > maxPoint ? `To move from ${targetSemesterPlan.oldCgpa.toFixed(2)} to ${targetSemesterPlan.target.toFixed(2)} by ${targetPeriod === "future" ? `the end of ${effectiveTargetEndStage}` : targetPeriod === "session" ? "the end of this session" : "the end of this semester"}, you would need an average GPA of ${targetSemesterPlan.requiredGpa.toFixed(2)} / ${maxPoint.toFixed(2)}, which is above the grading scale. Even a perfect ${maxPoint.toFixed(2)} GPA across these courses would put your CGPA at about ${targetSemesterPlan.maximumCgpa.toFixed(2)}.`
    ...
    : `To move from ${targetSemesterPlan.oldCgpa.toFixed(2)} to ${targetSemesterPlan.target.toFixed(2)} after ${targetPeriod === "future" ? `the end of ${effectiveTargetEndStage}` : targetPeriod === "session" ? "this session" : "this semester"}, you need approximately ${targetSemesterPlan.requiredGpa.toFixed(2)} / ${maxPoint.toFixed(2)} average GPA across ${targetSemesterPlan.units} course units. That is about ${Math.ceil(targetSemesterPlan.requiredPoints)} credit points across the target period.`;
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-900">
    ...
    {plannerMode && <section className="py-10"><div className="mx-auto max-w-7xl space-y-8 px-5 lg:px-8">
      ...
      {plannerMode === "target" && <section className="rounded-3xl border border-green-200 bg-green-50 p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="font-bold">Target Period<select value={targetPeriod} onChange={(e) => setTargetPeriod(e.target.value as "semester" | "session" | "future")} className="mt-2 w-full rounded-xl border bg-white px-4 py-3"><option value="semester">This Semester</option><option value="session">This Session</option><option value="future">Beyond This Session</option></select></label>{targetPeriod==="future"&&<label className="font-bold">Reach Target By<select value={effectiveTargetEndStage} onChange={(e)=>setTargetEndStage(e.target.value)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3">{targetFutureStages.map((level)=><option key={level} value={level}>End of {level}</option>)}</select></label>}</div>
        {targetPeriod==="future"&&<p className="mt-3 rounded-xl bg-green-100 p-3 text-sm font-bold text-green-900">Add the actual courses and units for each remaining semester from {startStage} {startTerm} through the end of {effectiveTargetEndStage}. The planner will calculate the average GPA you need across the whole period.</p>}

        {(targetPeriod==="future"
          ? targetFuturePeriods.map((period)=>({title:period.title,list:targetFutureCourses[period.key] || [{id:Date.now(),code:"",units:3}],setList:(updater: React.SetStateAction<TargetCourse[]>)=>setTargetFutureCourses((all)=>{const current=all[period.key] || [{id:Date.now(),code:"",units:3}];const next=typeof updater==="function"?(updater as (value:TargetCourse[])=>TargetCourse[])(current):updater;return {...all,[period.key]:next};})}))
          ...
          <div className="mt-2 space-y-3">{group.list.map((course,index)=><div key={course.id} className="grid grid-cols-[1fr_100px_38px] gap-2 rounded-xl bg-gray-50 p-2 sm:grid-cols-[1fr_130px_44px]"><input value={course.code} onChange={(e)=>group.setList((list)=>list.map((item)=>item.id===course.id?{...item,code:e.target.value.toUpperCase()}:item))} placeholder={`Course ${index+1} code`} className="min-w-0 rounded-xl border px-3 py-3"/><select value={course.units} onChange={(e)=>group.setList((list)=>list.map((item)=>item.id===course.id?{...item,units:Number(e.target.value)}:item))} className="rounded-xl border bg-white px-2 font-bold">{[1,2,3,4,5,6].map((unit)=><option key={unit} value={unit}>{unit} unit{unit>1?"s":""}</option>)}</select><button type="button" aria-label={`Delete ${course.code || `Course ${index+1}`}`} title="Delete course" onClick={()=>group.setList((list)=>list.length===1?[{id:Date.now()+Math.random(),code:"",units:3}]:list.filter((item)=>item.id!==course.id))} className="rounded-lg text-xl font-black text-red-600 hover:bg-red-50" >×</button></div>)}</div>
        ...
        {targetSemesterPlan.oldUnits > 0 && targetSemesterPlan.units > 0 && <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">Current CGPA</p><p className="mt-1 text-2xl font-black">{targetSemesterPlan.oldCgpa.toFixed(2)}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">Target CGPA</p><p className="mt-1 text-2xl font-black">{targetSemesterPlan.target.toFixed(2)}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">{targetPeriod==="semester"?"Semester GPA Needed":"Average GPA Needed"}</p><p className="mt-1 text-2xl font-black">{targetSemesterPlan.requiredGpa>maxPoint?"Not possible":Math.max(0,targetSemesterPlan.requiredGpa).toFixed(2)}</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">{targetPeriod==="semester"?"Semester Units":targetPeriod==="session"?"Session Units":"Target Period Units"}</p><p className="mt-1 text-2xl font-black">{targetSemesterPlan.units}</p></div></div>}</div>
        ...
        <p className="mt-4 text-xs leading-5 text-gray-600">Cumulative credit points are estimated from Current CGPA × Completed Units. Because the displayed CGPA may already be rounded, the result is an estimate.</p>
