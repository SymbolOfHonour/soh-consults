...
type System = "lasu" | "general" | "four-point";
...
export default function CgpaPlannerPage() {
  ...
  const activeGrades = system === "four-point" ? GRADES_4 : GRADES_5;
  const maxPoint = system === "four-point" ? 4 : 5;
  ...
  function resetPlan() { const first = makeSemester(0, entryMode); setSemesters([first]); setOpenSemester(first.id); setCurrentCgpa(""); setCtnup(""); setCtcp(""); setTargetCgpa(system === "four-point" ? "3.60" : "4.50"); window.localStorage.removeItem("soh-cgpa-planner-v3"); }
  ...
  async function downloadPdf() {
    ...
    pdf.setFontSize(8); pdf.setTextColor(107, 114, 128); pdf.text("This is a motivational projection, not an official university result. Confirm all calculations with your institution.", 15, Math.min(288, y + 8));
  ...
  async function downloadImage() {
    ...
    ctx.fillStyle = "#6b7280"; ctx.font = "17px Arial"; ctx.fillText("Motivational projection only. This is not an official university result.", 70, height - 45);
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-900">
    ...
    <section className="py-10"><div className="mx-auto max-w-7xl space-y-8 px-5 lg:px-8">
      <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8"><p className="text-sm font-black uppercase tracking-widest text-green-700">Student profile</p><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label className="font-bold">Mode of Entry<select value={entryMode} onChange={(e) => changeEntryMode(e.target.value as EntryMode)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3"><option value="utme">UTME</option><option value="direct-entry">Direct Entry</option></select></label><label className="font-bold">Programme Duration<select value={programmeYears} onChange={(e) => setProgrammeYears(e.target.value)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3">{(entryMode === "direct-entry" ? [3,4,5,6] : [4,5,6,7]).map((year) => <option key={year} value={String(year)}>{year} years</option>)}</select></label><label className="font-bold">Student Name (optional)<input value={studentName} onChange={(e) => setStudentName(e.target.value)} placeholder="Your name" className="mt-2 w-full rounded-xl border px-4 py-3" /></label><label className="font-bold">Programme (optional)<input value={programme} onChange={(e) => setProgramme(e.target.value)} placeholder="e.g. Marketing" className="mt-2 w-full rounded-xl border px-4 py-3" /></label></div><div className="mt-5 grid gap-3 rounded-2xl bg-green-50 p-4 text-green-950 sm:grid-cols-3"><div><p className="text-xs font-bold uppercase">Planning benchmark</p><p className="text-xl font-black">{projection.graduationUnits} units</p></div><div><p className="text-xs font-bold uppercase">Units left now</p><p className="text-xl font-black">{projection.unitsLeftNow}</p></div><div><p className="text-xs font-bold uppercase">After this projection</p><p className="text-xl font-black">{projection.unitsLeftAfterProjection} units left</p></div></div><p className="mt-3 text-xs leading-5 text-gray-500">Calculated at a planning minimum of 18 units per semester. Official graduation requirements vary by programme, faculty and institution.</p></section>

      <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8"><div className="flex flex-col justify-between gap-4 sm:flex-row"><div><p className="text-sm font-black uppercase tracking-widest text-green-700">Current record</p><h2 className="mt-1 text-2xl font-black">Where are you starting from?</h2></div><select value={system} onChange={(e) => setSystem(e.target.value as System)} className="h-fit rounded-xl border bg-white px-4 py-3 font-bold"><option value="lasu">LASU 5.0 System</option><option value="general">General Nigerian 5.0 System</option><option value="four-point">4.0 Grading System</option></select></div><div className="mt-5 grid gap-4 md:grid-cols-3"><label className="font-bold">Current CGPA<input type="number" min="0" max={maxPoint} step="0.01" value={currentCgpa} onChange={(e) => setCurrentCgpa(e.target.value)} placeholder="e.g. 3.79" className="mt-2 w-full rounded-xl border px-4 py-3" /></label><label className="font-bold">Completed Course Units (CTNUP)<input type="number" min="0" value={ctnup} onChange={(e) => setCtnup(e.target.value)} placeholder="e.g. 121" className="mt-2 w-full rounded-xl border px-4 py-3" /></label><label className="font-bold">Cumulative Credit Points (CTCP)<input type="number" min="0" step="0.01" value={ctcp} onChange={(e) => setCtcp(e.target.value)} placeholder="e.g. 458 (optional)" className="mt-2 w-full rounded-xl border px-4 py-3" /></label></div><p className="mt-4 rounded-xl bg-green-50 p-4 text-sm text-green-900">CTNUP is the total number of course units passed. CTCP is the sum of Unit x GP for all completed courses. Entering CTCP gives an exact projection; otherwise the rounded CGPA produces an estimate.</p></section>
