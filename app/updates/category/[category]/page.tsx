...
const categorySlug = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-");

export function generateStaticParams() {
  return Array.from(new Set(updates.map((item) => item.category))).map((category) => ({ category: categorySlug(category) }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { category } = await params;
  const name = updates.find((item) => categorySlug(item.category) === category)?.category;
  return name ? { title: `${name} Updates`, description: `Latest ${name.toLowerCase()} news and guidance from S.O.H CONSULTS.`, alternates: { canonical: `/updates/category/${category}` } } : { title: "Update Category" };
}
...
export default async function UpdateCategoryPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <section className="py-14"><div className="mx-auto max-w-5xl px-5 lg:px-8"><UpdatesExplorer initialCategory={name} /></div></section>
      <SiteContact />
    </main>
  );
}
