...
export default async function ImportedUpdatePage({ params }: Props) {
  ...
  return <main className="min-h-screen bg-gray-50 text-gray-900"><header className="border-b bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><a href="/"><img src="/soh-logo.jpg" alt="S.O.H CONSULTS" className="h-16 w-auto" /></a><a href={whatsappLink(`Hello S.O.H CONSULTS, I need help with: ${story.title}`)} target="_blank" rel="noopener noreferrer" className="rounded-full bg-green-700 px-5 py-3 text-sm font-black text-white">WhatsApp Us</a></div></header><section className="bg-gradient-to-br from-green-950 to-green-700 py-14 text-white"><div className="mx-auto max-w-4xl px-5"><a href="/updates" className="text-sm font-bold text-green-100">← Back to Latest Updates</a><p className="mt-6 text-sm font-black text-green-200">{story.category} · {story.institution}</p><h1 className="mt-4 text-3xl font-black leading-tight sm:text-5xl">{story.title}</h1><p className="mt-5 max-w-3xl leading-8 text-green-50">{story.summary}</p></div></section><section className="py-12"><div className="mx-auto max-w-4xl px-5"><article className="rounded-3xl border bg-white p-6 shadow-sm sm:p-10"><div className="space-y-5"><Body value={story.details} /></div>{story.deadline && <div className="mt-8 rounded-2xl bg-amber-50 p-5"><p className="text-xs font-black uppercase text-amber-800">Application deadline</p><p className="mt-2 text-lg font-black">{story.deadline}</p></div>}<div className="mt-8 rounded-2xl bg-green-50 p-6"><h2 className="text-xl font-black">Need registration assistance?</h2><a href={whatsappLink(`Hello S.O.H CONSULTS, I need assistance with: ${story.title}`)} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex rounded-xl bg-green-700 px-5 py-3 font-black text-white">Chat on WhatsApp</a></div><div className="mt-8 border-t pt-6"><p className="text-sm font-bold text-gray-500">Source</p><a href={story.source_url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex font-black text-green-700">Read the original report on {story.source_name} →</a></div><div className="mt-8 border-t pt-6"><ShareButtons url={pageUrl} title={story.title} /></div></article></div></section><SiteContact /></main>;
}
