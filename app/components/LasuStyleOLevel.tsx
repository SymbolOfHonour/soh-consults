...
export default function LasuStyleOLevel({subjects,grades,value,onChange,sittings,onSittingsChange,maxPerSitting=9,disabledTwo=false,note}:Props){
 ...
 const section=(sit:1|2,list:OLevelResultEntry[])=><div className="mt-6"><h3 className="text-sm font-black tracking-wide text-green-800">{sit===1?"FIRST SITTING":"SECOND SITTING"}</h3><div className="mt-3 space-y-3">{list.map((r,i)=><div key={i} className="grid gap-3 sm:grid-cols-[1fr_145px]"><label className="text-xs font-bold text-slate-600">Subject {i+1}<select className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 text-sm font-normal" value={r.subject} onChange={e=>edit(sit,i,"subject",e.target.value)}><option value="">Select subject</option>{subjects.map(s=><option key={s} value={s} disabled={([...first,...second].some((q)=>q!==r&&q.subject===s))}>{s}</option>)}</select></label><label className="text-xs font-bold text-slate-600">Grade<select className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-3 text-sm font-normal" value={r.grade} onChange={e=>edit(sit,i,"grade",e.target.value)}><option value="">Select grade</option>{grades.map(g=><option key={g}>{g}</option>)}</select></label></div>)}</div></div>;
