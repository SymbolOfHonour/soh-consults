...
export function writeSmartMeta(details:string,meta:SmartMeta){const clean=stripSmartMeta(details);const values:Object=Object.fromEntries(Object.entries(meta).filter(([,v])=>Boolean(v)&&v!=="auto"));if(!Object.keys(values).length)return clean;return `${clean}${clean?"\n\n":""}[SOH_SMART:${encodeURIComponent(JSON.stringify(values))}]`;}
