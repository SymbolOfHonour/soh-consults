...
export function automationSummary(story:QueuedStory){return{scheduled_for:getSchedule(story)?.toISOString()||null,expires_at:getExpiry(story)?.toISOString()||story.deadline_iso||null}}

export async function runContentAutomation(now=new Date()){
 const stories=await listStories();let published=0,archived=0;
 for(const story of stories){
  const schedule=getSchedule(story);const expiry=getExpiry(story)||(story.deadline_iso?new Date(story.deadline_iso):null);
